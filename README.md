...
전체 E2E는 `can-utils`와 `vcan0`을 준비하고 gateway 및 ohayessOS runtime을 빌드한 뒤 `./scripts/e2e-vcan.sh <ohayess-runtime 경로>`로 실행합니다. 테스트는 opendbc 기반 Genesis G80 프레임과 CGW1 저빔·LVR12 기어·WHL_SPD11 휠 속도·SCC14 신호를 보내고 runtime의 최신 protobuf `VehicleState`에 속도 약 22.22 m/s, 종가속도 1.25 m/s², 조향각 π/2 rad, 브레이크 입력, `night_mode`, Park 기어, 네 휠 속도, SCC 상태가 함께 누적되는지 확인합니다.
