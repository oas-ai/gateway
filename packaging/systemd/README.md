...
이 패키지는 read-only `oas-gateway-host`, `ohayess-runtime`, `ohayess-hmi`를 한 감독 프로세스로 실행한다. Gateway snapshot은 runtime과 loopback HMI에 동시에 fan-out된다. 하나가 종료되면 모두 새 stream으로 재연결한다. 재시작 간격은 1초에서 시작해 5초를 넘지 않으며, 30초 이상 정상 실행하면 다시 1초로 초기화한다.
...
sudo install -m 0755 /path/to/ohayess-hmi /usr/lib/oas-gateway/bin/
